...
const StressTensor = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-[calc(100vh-64px)] bg-slate-900 text-slate-200 overflow-hidden font-sans relative">
      <div className={\lex flex-col w-full md:w-96 bg-slate-900 border-r border-slate-800 shadow-xl z-20 flex-shrink-0 transition-transform duration-300 \\}>
        <div className="flex-1 overflow-y-auto custom-scrollbar p-4 flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-3 p-3 bg-slate-800/30 border border-slate-700 rounded-lg">
             ...
             <div className="flex flex-col gap-2">
               {(['x', 'y', 'z'] as const).map(axis => (
                 <div key={axis} className="flex items-center gap-3">
                   ...
                   <input type="range" min="-180" max="180" step="1" value={rotation[axis]} onChange={(e) => setRotation({...rotation, [axis]: parseFloat(e.target.value)})} className={\lex-1 accent-\-500\} />
                 </div>
               ))}
             </div>
             ...
             <div className="mt-2 p-3 bg-amber-950/20 rounded border border-amber-900/30">
               ...
               <p className="text-[10px] text-slate-500 mt-3 leading-relaxed">
                 Si rotas en 3D hasta eliminar todos los cortantes (<InlineMath tex="\tau'_{ij} \approx 0" />), la diagonal te dará los <strong>Esfuerzos Principales</strong> (Autovalores reales del espacio).
               </p>
             </div>
          </div>
          ...
          <div className="flex flex-col gap-2 mt-2">
            <label className="flex items-center gap-3 cursor-pointer p-3 bg-indigo-900/20 border border-indigo-500/30 rounded-lg hover:bg-indigo-900/40 transition">
              ...
              <div className={\elative w-10 h-6 transition-colors rounded-full \\}>
